...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            if wandb:
                log_payload = {"loss": current_loss, "logits_loss": current_logits_loss, "aux_loss": current_aux_loss, "learning_rate": current_lr, "epoch_time": eta_min}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_hidden_layers', default=32, type=int, help="隐藏层数量")
    ...
    parser.add_argument('--mhc_sublayer_prenorm', default=1, type=int, choices=[0, 1], help="Ablation：sublayer F 内部是否加 RMSNorm（use_mhc=1 时生效）。1=F(z)=sublayer(RMSNorm(z))（默认，论文 §3.2 pre-norm Transformer + Table 3 注释明示）；0=F(z)=sublayer(z) 直接接收 raw H^pre·x（深层 bf16 易爆 NaN，仅用于 A-B 对照）")
    parser.add_argument('--max_seq_len', default=512, type=int, help="训练的最大截断长度（中文1token≈1.5~1.7字符）")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="是否使用MoE架构（0=否，1=是）")
    parser.add_argument("--data_path", type=str, default="/mnt/tidalfs-bdsz01/dataset/aigc-data/data/minimind_dataset/pretrain_t2t.jsonl", help="预训练数据路径")
    parser.add_argument('--from_weight', default='none', type=str, help="基于哪个权重训练，为none则从头开始")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="是否自动检测&续训（0=否，1=是）")
    parser.add_argument("--use_wandb", action="store_true", help="是否使用wandb")
    parser.add_argument("--wandb_project", type=str, default="MiniMind-Pretrain", help="wandb项目名")
    parser.add_argument("--wandb_exp", type=str, default="Exp", help="wandb实验名")
    ...
    lm_config = MiniMindConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers,
                               ...
                               mhc_sublayer_prenorm=bool(args.mhc_sublayer_prenorm),
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"Exp-{args.wandb_exp}-Epoch-{args.epochs}-BatchSize-{args.batch_size*args.accumulation_steps}-LearningRate-{args.learning_rate}"
    ...
    Logger(f'========== Model Architecture ==========\n{model}')
    Logger(f'========== Param Stats ==========')
    Logger(f'Total:     {n_params/1e6:8.2f}M ({n_params:,})')
    Logger(f'Trainable: {n_trainable/1e6:8.2f}M ({n_trainable:,})')
    Logger(f'=========================================')
